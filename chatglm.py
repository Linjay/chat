...
def event():
    ...
    if conversationId not in chat_holder.keys():
        print("invalid conversation:"+conversationId)
    json_message={
        "history": chat_holder[conversationId]['history'],
    ...
    try:
        ...
        chat_holder[conversationId]['history'] = chatgpt_response.json()['history']
    ...
    json_sendmessages={"msgtype": "text","text": {"content":asker+"你好!\r\n"+answer+'\r\nconversation:'+str(len(chat_holder[conversationId]['history']))}}
    url='https://oapi.dingtalk.com/robot/send?access_token='+chat_holder[conversationId]['dingToken']
    response=requests.post(url,headers=headers2,json=json_sendmessages)
    print(response.text)

    if len(chat_holder[conversationId]['history'])>16:
        chat_holder[conversationId]['history']=[]
